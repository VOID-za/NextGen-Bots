...
module.exports = {
  name: "fire",
  aliases: ["f"],
  description: "Fire a bot reviewer at NextGen Bots.",
  async run(client, message, args) {
    ...
    try {
      kickUser.kick({
        reason: `This user has been fired from NextGen Bots by: ${message.author.username}.`,
      });
    } catch (err) {}

    const embed = new EmbedBuilder()
      .setTitle("You have been fired from NextGen Bots.")
      .setDescription(
        `Think we make a mistake? Contact an admin for more information.`
      )
      .addFields({ name: "Reason Given", value: `${reason}`, inline: true })
      .setFooter({
        text: "NextGen Bots - Staff Management",
    ...
    const logEmbed = new EmbedBuilder()
      ...
      .setDescription(`${member.user} has been fired from NextGen Bots.`)
      .addFields({
      ...
      .addFields({ name: "Reason Given", value: `${reason}`, inline: true })
      .setFooter({
        text: "NextGen Bots - Staff Management Logs",
        iconURL: client.user.displayAvatarURL(),
      });
    client.channels.resolve("948453723166896149").send({ embeds: [logEmbed] });

    message.reply(
      `<:yes:946581450852016198> ${user} has been fired from NextGen Bots.`
    );
  },
};