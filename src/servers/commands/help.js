...
module.exports = { 
  data: new SlashCommandBuilder()
    .setName("help") 
    .setDescription(
      "Replies with information about NextGen Bots, as well as how to add your server."
    ),
  async execute(interaction) {
    const embed = new EmbedBuilder()
      ...
      .setFooter({
        text: `NextGen Bots - Help Command`,
        iconURL: `${global.sclient.user.displayAvatarURL()}`,
      });

    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setURL(`https://localhost:3000/servers`)
        .setLabel("Visit Universe Servers")
        .setStyle(ButtonStyle.Link),
      new ButtonBuilder()
        .setURL(`https://discord.gg/DKVkScDQMc`)
