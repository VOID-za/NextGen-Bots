  ...
  const addEmbed = new EmbedBuilder()
    ...
    .setDescription(
      "<:add:946594917596164136> " +
        bot.tag +
        " has been submitted to NextGen Bots."
    )
    .setColor("Blue")
    .addFields({
      name: "Bot",
...
app.post("/bots/:id/edit", checkAuth, async (req, res) => {
  ...
  if (
    ...
    const editEmbed = new EmbedBuilder()
      .setTitle("Bot Edited")
      .setDescription(
        ":pencil: " + bot.tag + " has been edited on NextGen Bots."
      )
      .setColor("Yellow")
      .addFields({
        name: "Bot",
...
app.post("/bots/:id/delete", checkAuth, async (req, res) => {
  ...
  if (
    ...
    if (kickBot) {
      kickBot.kick("Deleted from NextGen Bots.");
    }
    await botm.delete();
    ...
    const editEmbed = new EmbedBuilder()
      ...
      .setDescription(
        "<:ul_no:946581450600370298> " +
          bot.tag +
          " has been deleted on NextGen Bots."
      )
      .setColor("Red")
      .addFields({
        name: "Bot",
...
app.post("/bots/:id/report", checkAuth, async (req, res) => {
  ...
  if (member) {
    ...
    const reportEmbed = new EmbedBuilder()
      ...
      .setDescription(
        ":rotating_light: " + bot.tag + " has been reported on NextGen Bots."
      )
      .setColor("fe3c3c")
      .addFields({
        name: "Bot",
...
app.post("/bots/:id/vote", checkAuth, async (req, res) => {
  ...
  const votedEmbed = new EmbedBuilder()
    .setTitle("Bot Voted")
    .setDescription(
      `<:vote:1043639183991898203> ${bot.name}#${bot.discriminator} has been voted on NextGen Bots.`
    )
    .setColor("#7289da")
    .addFields(
      {
...
app.post("/bots/:id/deny", checkAuth, checkStaff, async (req, res) => {
  ...
  if (bot.approved === true) {
    return res.status(400).json({
      message: "This bot is already approved on NextGen Bots.",
    });
  }

  if (bot.denied === true) {
    return res.status(400).json({
      message: "This bot is already denied on NextGen Bots.",
    });
  }
  ...
  const denyEmbed = new EmbedBuilder()
    ...
    .setDescription(
      "<:no:946581450600370298> " +
        bot.tag +
        " has been denied on NextGen Bots."
    )
    .setColor("Red")
    .addFields({
      name: "Bot",
...
app.post("/bots/:id/testing", checkAuth, checkStaff, async (req, res) => {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({
      text: "Testing Session - NextGen Bots",
      iconURL: `${global.client.user.displayAvatarURL()}`,
    });
...
app.use("/bots/:id/status", checkAuth, checkStaff, async (req, res) => {
  ...
  if (req.method === "POST") {
    ...
    const approveEmbed = new EmbedBuilder()
      ...
      .setDescription(
        "<:yes:946581450852016198> " +
          bot.tag +
          " has been approved on NextGen Bots."
      )
      .setColor("Green")
      .addFields({
        name: "Bot",
      ...
      .setFooter({
        text: "Approve Logs - NextGen Bots",
        iconURL: `${global.client.user.displayAvatarURL()}`,
      });
    ...
    let guild = client.guilds.cache.get(global.config.guilds.testing);
    const kickBot = guild.members.cache.get(bot.id);
    kickBot.kick("Approved on NextGen Bots.");
    let channel = guild.channels.cache.find(
      (c) => c.name == channelName.toLowerCase()
    );
    if (channel) channel.delete("This bot was approved on NextGen Bots.");
    return res.redirect(
...
app.get("/discord", (_req, res) =>
  res.redirect("https://discord.gg/DKVkScDQMc")
);
...
app.get("/newbot", (_req, res) =>
  res.redirect("http://localhost:3000/bots/new")
);
